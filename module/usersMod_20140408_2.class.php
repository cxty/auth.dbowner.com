...
class usersMod extends commonMod{
	 ...
	 * 检验用户是否有权限访问接口
	 */
	private function checkUserIsValid($format, $token ,$cacheIdent){	
		//不是内部应用
		if ( !in_array($token['client_id'], $this->config['oauth']['login']) ) {
			//判断是否是默认权限
			$dbSoapInterface = new DB_SoapInterface();
			
			//用信息缓存
			$memKey = '|users|checkUserIsValid|getAuthListInfo-1|' . $cacheIdent. '-';
			$menVal = $this->_Cache->get( $memKey );
			if ( $menVal ) {
				$authArr = $menVal;
			} else {
				$authArr = $dbSoapInterface->getAuthListInfo();
				$this->_Cache->set( $memKey, $authArr, $this->config['MEM_EXPIRE'] );
			}
			...
			if ( !$_isDefault ) {
				...
				//数据缓存
				$memKey = '|users|checkUserIsValid|getPermValue-1|' . $cacheIdent. '-';
				$menVal = $this->_Cache->get( $memKey );
				if ( $menVal ) {
					$permValue = $menVal;
				} else {
					$permValue = $mandOAuthLog->getPermValue($token);
					$this->_Cache->set( $memKey, $permValue, $this->config['MEM_EXPIRE'] );
				}
	...
	private function checkAccess($format,$access_token){	
		//用信息缓存
		$memKey = '|users|checkAccess|getTokenInfo-1|' . $access_token . '-';
		$menVal = $this->_Cache->get( $memKey );
		if ( $menVal ) {
			$tokenInfo = $menVal;
		} else {
			...
			$this->_Cache->set( $memKey, $tokenInfo, $this->config['MEM_EXPIRE'] );
		}
		...
		//用信息缓存
		$memKey = '|users|checkAccess|checkAccessToken-1|' . $access_token . '-';
		$menVal = $this->_Cache->get( $memKey );
		if ( $menVal ) {
			$re = $menVal;
		} else {
			...
			$this->_Cache->set( $memKey, $re, $this->config['MEM_EXPIRE'] );
		}
		...
		if($re['error'] != 'ok'){
			...
			//用信息缓存
			$memKey = '|users|checkAccess|checkAuthPastDue-1|' . $access_token . '-';
			$menVal = $this->_Cache->get( $memKey );
			if ( $menVal ) {
				$backArr = $menVal;
			} else {
				...
				$this->_Cache->set( $memKey, $backArr, $this->config['MEM_EXPIRE'] );
			}
	...
	private function checkrefresh($format,$refresh_token){
		...
		//用信息缓存
		$memKey = '|users|checkrefresh|getTokenInfo-1|' . $refresh_token . '-';
		$menVal = $this->_Cache->get( $memKey );
		if ( $menVal ) {
			$tokenInfo = $menVal;
		} else {
		...
		//用信息缓存
		$memKey = '|users|checkrefresh|checkRefreshToken-1|' . $refresh_token . '-';
		$menVal = $this->_Cache->get( $memKey );
		if ( $menVal ) {
			$re = $menVal;
		} else {
		...
		if($re['error'] != 'ok'){
			...
			//用信息缓存
			$memKey = '|users|checkrefresh|checkAuthPastDue-1|' . $refresh_token . '-';
			$menVal = $this->_Cache->get( $memKey );
			if ( $menVal ) {
				$re = $menVal;
			} else {
	...
	public function test () {
		$re = array('test' => $memKey_result);
		echo json_encode($re);exit;
		ComFun::pr($this->_Cache);exit;
		$access_token = $_GET ['access_token'] ? $_GET ['access_token'] : $_POST ['access_token'];
		$access_token = 'aaa';
		$memKey_result = '|auths|test||' . $access_token . '-';
		
		$menVal = $this->_Cache->get( $memKey_result );
		if ( $menVal ) {
			echo json_encode($menVal);exit;
		}
	...
	public function show(){	
		...
		//权限检验
		$token['port'] = 'show';
		$this->checkUserIsValid($format, $token, $access_token);
	...
	public function fresh_token(){	
		...
		$this->checkUserIsValid($format, $token, $refresh_token);
