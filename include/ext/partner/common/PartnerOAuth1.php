...
class PartnerOAuth1 { 
     ...
     */ 
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL, $host=NULL) { 
    ...
    /** 
     * Get a request_token from partner 
     * 
     * @return array a key/value array containing oauth_token and oauth_token_secret 
     */ 
    function getRequestToken($requestTokenURL,$seArr=false,$oauth_callback = NULL) {  	
        $parameters = array(); 
        
   		if($seArr){		
			$parameters = array_merge($parameters,$seArr);
		}
	...
	function getAuthorizeURL($accessTokenURL,$token) { 
		if (is_array($token)) { 
			$token = $token['oauth_token']; 
		}
			
		return $accessTokenURL . "?oauth_token=".$token; 
    } 
    function getAuthorizeURL_Tianya($accessTokenURL,$token, $callback){
    	if (is_array($token)) {
    		$token = $token['oauth_token'];
    	}
    	return $accessTokenURL . "?oauth_token={$token}&consumer_key=".$this->consumer->key."&oauth_callback=" . urlencode($callback);
    }
