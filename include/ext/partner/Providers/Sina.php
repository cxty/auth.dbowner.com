 ...
 *
 */
class Providers_Sina{
	
	var $format = 'json';
	
	public function __construct($host,$api_key,$api_sercet,$OAuthArr){
		$this->host                = $host;
		...
		$this->oauth = new PartnerOAuth2($this->api_key,$this->api_sercet,$this->access_token,$this->refresh_token,$this->host);
	...
	public function follow( $fieldArr ) {
		...
		return $this->oauth->post( $this->host . 'friendships/create' . '.' . $this->format , $params ? $params : array() );
	}
	...
	public function getFriendList( $fieldArr ) {
		...
		return $this->oauth->get( $this->host . 'friendships/friends' . '.' . $this->format , $params ? $params : array() );
	}
...
//未使用的
	 ...
	 *   原方法名：friends
	 */
	function getFriends( $cursor = false , $count = false , $uid_or_name = null )
	{
		return $this->oauth->post( $this->host.'friendships/friends'.'.'.$this->format ,  $uid_or_name , false , $count , $cursor );
	}
	...
	{
		return $this->oauth->post( 'http://api.t.sina.com.cn/statuses/followers.json' ,  $uid_or_name , false , $count , $cursor );
	}
