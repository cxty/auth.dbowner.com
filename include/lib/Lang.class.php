...
class Lang
{
	...
	static public function init($config=array())
	{
		...
		self::$config['LANG_PACK_ORIG'] = self::$config['LANG_PACK_PATH']; //原始语言包路径
   ...
   static public function get($key,$pack='')
    ...
    {		
			...
			if ( self::$config['LANG_DEFAULT'] ) {
				$lang = self::$config['LANG_DEFAULT'];
			}
...
//echo $lang_file;exit;
	 ...
	 * 取当前语言值
	 */
	static public function getAssign($key,$lang='zh',$pack=''){
		$assign_array=self::getAssignPack($lang,$pack);
		return isset($assign_array[$key])?$assign_array[$key]:$key;
	}
	/**
	 * 取当前语言数组
	 */
	static public function getAssignPack($lang='zh',$pack=''){
		$common_pack=self::$config['LANG_PACK_COMMON'];//公用语言包
		$pack=empty($pack)?$common_pack:$pack;//不指定语言包，自动调用公共语言包
		
		if(!isset(self::$assign_array[$pack]))
		{
			if($pack==$common_pack)
			{
		
				self::$assign_array[$pack]=self::_loadAssingLangPack($common_pack,$lang);
		
			}
			else//加载公共语言包和指定的语言包
			{
				self::$assign_array[$pack]=@array_merge(self::_loadAssingLangPack($common_pack,$lang),self::_loadAssingLangPack($pack,$lang));//两个数组合并
			}
		}
		
		return self::$assign_array[$pack];
	}
	/**
	 * 获取指定语言包数组
	 */
	static private function _loadAssingLangPack($pack,$lang='zh'){
		$lang_array=array();
		$lang_file=self::$config['LANG_PACK_ORIG'].$lang.'/'.$pack.self::$config['LANG_PACK_SUFFIX'];

		if(file_exists($lang_file))
		{
			$assign_array=require($lang_file);
			if(!is_array($assign_array))//数据不为数组的时候，为了下面的array_merge，赋值为空数组，
			{
				$assign_array=array();
			}
		}
