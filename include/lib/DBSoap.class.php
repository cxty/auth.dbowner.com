...
class DBSoap{
	...
	public function __construct(){
		...
		$this->Soap_Client = $GLOBALS['config']['DES']['Soap_Client'];
		$this->Soap_Header = $GLOBALS['config']['DES']['Soap_Header'];
	}
	/**
	 * 载入加密文件
	 */
	...
	private function _soap($platform){
		switch(strtolower($platform)){
			...
			case 'auth':
				...
				break;
			case 'plus':
				$this->Soap_Client = $GLOBALS['config']['DES']['Soap_Client_Plus'];
				$this->Soap_Header = $GLOBALS['config']['DES']['Soap_Header_Plus'];
				break;
			case 'pay':
			...
			case 'ads':
				...
				break;
			default:
				$this->Soap_Client = $GLOBALS['config']['DES']['Soap_Client_tAuth'];
				$this->Soap_Header = $GLOBALS['config']['DES']['Soap_Header_tAuth'];
				break;
		}
	}
	/**
...
//		ComFun::pr($cArr);
		try{	
			...
			$a = array('data'=>json_encode(array('data'=>$val)));
			$a = $client->$className($a)->return;
			$_re =  json_decode($a);
